# ThreeBody
这是一个可以画一些神秘章鱼怪的小工具。
这个简化的三体问题总共输入参数有12个，即为三个天体的x/y/vx/vy，根据平移对称性、旋转对称性，你可以固定其中五个不动，剩下还有七个参数，这就是个七维的参数空间。然后你可以取一些切面，把这个二维切面，对应的参数全部计算一遍，记录对应参数在失衡之前会存活多少帧，然后归一化，可视化，这样就得到了这些，在我面前扭动的大章鱼。我反复确认过，这大概不是计算误差。欧拉法和rk4的计算有些区别，但差的并不是特别大，由此才敢抛砖引玉。
